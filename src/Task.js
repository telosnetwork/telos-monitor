...
export default class Task {
    ...
    async notify(task_id){
        ...
        const last_tasks = await this.pool.query(
            "SELECT * FROM task_status WHERE task = $1 AND type = $2 AND checked_at > now() - interval '24 hours' AND checked_at < now() - interval '1 minute' AND message IN ($3) ORDER BY id DESC LIMIT 2",
